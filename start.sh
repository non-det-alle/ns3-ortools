...
# the ns3 user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the ns3 user to the desired the user (NB_USER)
    if id ns3 &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" ns3 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the ns3 user:"
            _log "- username: ns3       -> ${NB_USER}"
            _log "- home dir: /home/ns3 -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the ns3 user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the ns3 home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "ns3" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/ns3 to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/ns3/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/ns3 to /home/${NB_USER}!"
                _log "Attempting to symlink /home/ns3 to /home/${NB_USER}..."
                if ln -s /home/ns3 "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/ns3 to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/ns3/"* ]]; then
    ...
    # Prepend /home/${NB_USER}/.local/bin to sudo secure_path
    sed -r "s#Defaults\s+secure_path\s*=\s*\"?([^\"]+)\"?#Defaults secure_path=\"/home/${NB_USER}/.local/bin:\1\"#" /etc/sudoers | grep secure_path > /etc/sudoers.d/path
...
# user we started as.
    ...
    ns3_UID="$(id -u ns3 2>/dev/null)"  # The default UID for the ns3 user
    ns3_GID="$(id -g ns3 2>/dev/null)"  # The default GID for the ns3 user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old ns3 user to sn3 ($(id -u ns3):$(id -g ns3))"
            ...
            sed --expression="s/^ns3:/sn3:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/ns3:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "ns3" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/ns3. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/ns3 ]]; then
        _log "WARNING: no write access to /home/ns3. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
